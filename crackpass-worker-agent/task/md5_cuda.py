...
from libs import md5_cuda_stubs

from worker_task import WorkerTask


class MD5Cuda(WorkerTask):
    ...
    def working_loop(self, ):
        ...
        """
        #--- init
        md5_cuda_stubs.init(self.task_id,
                            self.occupied_cpu,
                            self.params['hash_str'],
                            self.params['charset_str'])
        self.md5_cuda_stubs.start(self.task_id)
        ...
        self.task_state = self.md5_cuda_stubs.get_state(self.task_id)
        #--- main loop
        while self.task_state != WorkerTask.C_TASK_STATE['TASK_STATE_STOPPED']      and \
            ...
            # Result found
                ...
                res_str_lst = []
                self.md5_cuda_stubs.pop_result(res_str_lst)
                # Report it to master
