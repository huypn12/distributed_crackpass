...
__device__ inline int PBKDF2(
        int S[],int saltLen,
        int stored_pvv[],int dkLen,
        int *P, int passLen,
        unsigned char *Key
        )
{
...
void RunKernel(
        int base,
        char *devPass, char *devPtr,
        int width, int qualities, int wordCount,
        char *pre_terminal, int pre_terminal_len,
        int *salt, int saltLen,
        int *stored_pvv, int dkLen,
        unsigned char *data, unsigned int len,
        unsigned char *out, unsigned char extension, unsigned char *Key,
        int* ret,
        fcrypt_ctx *zcx, aes_ctx *encr_ctx,
        z_stream *strm, struct inflate_state FAR *d_state
        )
{	
    int index = 0,ok=1;
    int begin;
